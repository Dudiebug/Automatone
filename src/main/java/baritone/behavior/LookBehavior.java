...
public final class LookBehavior extends Behavior implements ILookBehavior {
    ...
    /** Applies a server-owned target during the real runtime tick. */
    public void applyServerTick() {
        if (this.target == null || this.target.mode != Target.Mode.SERVER) {
            return;
        }
    ...
    private static abstract class AbstractAimProcessor implements ITickableAimProcessor {
        ...
        private float mouseToAngle(double mouseDelta) {
            // Server-side hosts have no mouse setting. Keep the vanilla default
            // quantization so the native aim processor remains deterministic.
            final double f = 0.5d * 0.6d + 0.2d;
