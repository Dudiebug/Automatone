...
public class MovementDescend extends Movement {
    ...
    public MovementState updateState(MovementState state) {
        ...
        state.setInput(Input.SNEAK, baritone.getSettings().allowWalkOnMagmaBlocks.value && ctx.world().getBlockState(ctx.player().blockPosition().below()).is(Blocks.MAGMA_BLOCK));
    ...
    public boolean safeMode() {
        ...
        for (int y = 0; y <= 2; y++) { // we could hit any of the three blocks
            if (MovementHelper.avoidWalkingInto(BlockStateInterface.get(ctx, into.above(y)), baritone.getSettings())) {
                return true;
            }
        }
        return false;
