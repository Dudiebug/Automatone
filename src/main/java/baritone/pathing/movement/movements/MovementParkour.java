...
public class MovementParkour extends Movement {
    ...
    public static void cost(CalculationContext context, int x, int y, int z, Direction dir, MutableMoveResult res) {
        ...
        if (MovementHelper.avoidWalkingInto(adj, context.settings) && !(adj.getFluidState().getType() instanceof WaterFluid)) { // magma sucks
            return;
        }
    ...
    private static boolean checkOvershootSafety(BlockStateInterface bsi, int x, int y, int z) {
        ...
        return !MovementHelper.avoidWalkingInto(bsi.get0(x, y, z), bsi.settings) && !MovementHelper.avoidWalkingInto(bsi.get0(x, y + 1, z), bsi.settings);
    }
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (baritone.getSettings().allowWalkOnMagmaBlocks.value && ctx.world().getBlockState(ctx.playerFeet().below()).is(Blocks.MAGMA_BLOCK)) {
