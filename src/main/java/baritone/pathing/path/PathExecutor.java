...
public class PathExecutor implements IPathExecutor, Helper {
    ...
    public boolean onTick() {
        ...
        BlockStateInterface bsi = new BlockStateInterface(ctx, false, behavior.baritone.getSettings());
    ...
    private static boolean sprintableAscend(IPlayerContext ctx, MovementTraverse current, MovementAscend next, IMovement nextnext) {
        if (!ctx.getSettings().sprintAscends.value) {
            return false;
        }
        ...
        if (MovementHelper.avoidWalkingInto(ctx.world().getBlockState(current.getSrc().above(3)), ctx.getSettings())) {
            return false;
        }
        return !MovementHelper.avoidWalkingInto(ctx.world().getBlockState(next.getDest().above(2)), ctx.getSettings()); // codacy smh my head
    }
    ...
    private static boolean canSprintFromDescendInto(IPlayerContext ctx, IMovement current, IMovement next) {
        ...
        return next instanceof MovementDiagonal && ctx.getSettings().allowOvershootDiagonalDescend.value;
    }
