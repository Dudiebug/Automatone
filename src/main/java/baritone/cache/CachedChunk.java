...
// This should really not need to happen
    ...
    CachedChunk(int x, int z, int height, BitSet data, BlockState[] overview, Map<String, List<BlockPos>> specialBlockLocations, long cacheTimestamp) {
        ...
        this.specialBlockLocations = copySpecialBlockLocations(specialBlockLocations);
        this.cacheTimestamp = cacheTimestamp;
        if (this.specialBlockLocations.isEmpty()) {
            this.special = null;
        } else {
    ...
    private static Map<String, List<BlockPos>> copySpecialBlockLocations(Map<String, List<BlockPos>> source) {
        Map<String, List<BlockPos>> copy = new LinkedHashMap<>();
        for (Map.Entry<String, List<BlockPos>> entry : source.entrySet()) {
            List<BlockPos> positions = new ArrayList<>(entry.getValue().size());
            for (BlockPos pos : entry.getValue()) {
                positions.add(new BlockPos(pos.getX(), pos.getY(), pos.getZ()));
            }
            copy.put(entry.getKey(), List.copyOf(positions));
        }
